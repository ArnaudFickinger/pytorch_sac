...
class RandomImageSource(ImageSource):
    ...
    def build_arr(self):
        ...
        for i in range(self.total_frames):
            ...
            self.arr[i] = cv2.resize(im, (self.shape[1], self.shape[0])).reshape((self.shape[1], self.shape[0], 1)) if self.grayscale else cv2.resize(im, (self.shape[1], self.shape[0])) ## THIS IS NOT A BUG! cv2 uses (width, height)
